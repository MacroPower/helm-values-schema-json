...
func TestGenerateJsonSchema_AdditionalPropertiesRecursive(t *testing.T) {
	tests := []struct {
		name                    string
		additionalPropertiesSet bool
		additionalProperties    bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			config := &Config{
				Input:      []string{"../testdata/basic.yaml"},
				OutputPath: "../testdata/basic.schema.json",
				Draft:      2020,
				Indent:     4,
				SchemaRoot: SchemaRoot{
					ID:                   "",
			...
			generatedSchemaProperties := generatedSchema["properties"].(map[string]interface{})
			generatedSchemaImage := generatedSchemaProperties["image"].(map[string]interface{})

			if tt.expected == nil {
				_, exists := generatedSchemaImage["additionalProperties"]
				assert.False(t, exists, "additionalProperties should not be present in the generated schema")
			} else {
				assert.Equal(t, tt.additionalProperties, generatedSchemaImage["additionalProperties"], "additionalProperties value mismatch")
			}
